...
namespace Pub.Models
{
    public class Verify
    {
        public Stu_JSON Date(string acad, string phase, string connectionString)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string cmd = $"SELECT [syear], [sem] FROM [scoparams] WHERE ('{DateTime.Now.ToString("yyyMMdd", culture)}' BETWEEN [open_date] AND [end_date]) AND [phase] = '{phase}' AND [acadno] = '{acad}'";
                using (SqlCommand command = new SqlCommand(cmd, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new Stu_JSON()
                            {
                                syearEnd = Int32.Parse(reader.GetString(0)),
                                semEnd = Int32.Parse(reader.GetString(1))
                            };
                        }
                        else
                        {
        ...
        public string Deptno(string stuno, int syearEnd, int semEnd, string connectionString)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                string cmd = $"SELECT [deptno] FROM [regstusem] WHERE [stuno] = '{stuno}' AND [syear] = '{syearEnd}' AND [sem] = {(semEnd > 2 ? 2 : semEnd)}";
                using (SqlCommand command = new SqlCommand(cmd, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
        ...
        public (bool, string) Identity(string WebPid1, string program_no, string connectionString)
        {
            ...
            if (prog_group == "7")
            {
                ...
                else
                {
                    ...
                    bool check = false;
                    string msg = "";
                    (check, msg) = Agent(id, program_no, connectionString);
                    if (check)
                    {
                        return Check(msg, program_no, connectionString);
                    }
                    else
                    {
                        return (false, msg);

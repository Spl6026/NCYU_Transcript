...
namespace Transcript.Models
{
    public class DataBase
    {
        ...
        string informGrade(Decimal grade, string pass, int drop, int arrival)
        {
            if (drop == 1)
                return "停修";
        ...
        public Tuple<List<informStudent>, List<informCourses>> InformGet(string StudentId, int syearEnd, int semEnd, string connectionString)
        {
            TR = 0;
            List<informStudent> stu = new List<informStudent>();
            List<informCourses> course = new List<informCourses>();
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                ...
                cmd = $"SELECT [chineseco], [credit], [pass], [totalscore], [capacity], [dropcd], [arrival_cd], CASE WHEN [cdomainna] LIKE '%通識%' THEN '通識' WHEN [smn] = 1 THEN '必修' WHEN [smn] = 2 THEN '選修' END FROM [selstch] LEFT JOIN [crscourse] ON [selstch].[cono] = [crscourse].[cono] LEFT JOIN [crsdomain] ON [crsdomain].[domainno] = [selstch].[seldomain] WHERE [selstch].[stuno] = '{StudentId}' AND [selstch].[syear] = {syearEnd} AND [selstch].[sem] = {semEnd} UNION SELECT [chineseco], [credit], CAST('Y' AS CHAR) AS [pass], NULL AS [totalscore], CAST('0' AS CHAR) AS [capacity], CAST('0' AS CHAR) AS [dropcd], CAST('1' AS CHAR) AS [arrival_cd], CASE WHEN [cdomainna] LIKE '%通識%' THEN '通識' WHEN [smn] = 1 THEN '必修' WHEN [smn] = 2 THEN '選修' END FROM [crscredit] LEFT JOIN [crscourse] ON [crscredit].[crcono] = [crscourse].[cono] LEFT JOIN [crsdomain] ON [crsdomain].[domainno] = [crscredit].[seldomain] WHERE [crscredit].[stuno] = '{StudentId}' AND [crscredit].[syear] = {syearEnd} AND [crscredit].[sem] = {semEnd} AND ([cr_class] = 2 {cr}) ORDER BY [capacity]";
                using (SqlCommand command = new SqlCommand(cmd, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            {
                                course.Add(new informCourses()
                                {
                                    Course = reader.IsDBNull(0) ? "-" : reader.GetString(0),
                                    Credit = reader.GetDecimal(1),
                                    Pass = reader.GetString(2).Trim() == "Y",
                                    Grade = reader.IsDBNull(3) ? TR_Count(reader.GetDecimal(1)) : informGrade(reader.GetDecimal(3), reader.GetString(2), Int32.Parse(reader.GetString(5)), Int32.Parse(reader.GetString(6))),
                                    Capacity = Int32.Parse(reader.GetString(4)),
                ...
                string cNumber = "一二三四五六七八九";
                cmd = $"SELECT [acadbrf], [colnam], [deptnam], [regstusem].[grade], [codsc], [cname], [rgcrd], [scoavg], [total_score] FROM [stufile] LEFT JOIN [pubedusys] ON [pubedusys].[acadno] = '{acad}' LEFT JOIN [sclperson] ON [stufile].[idno] = [sclperson].[idno] LEFT JOIN [pubdep] ON [stufile].[deptno] = [pubdep].[deptno] LEFT JOIN [pubcol] ON [pubcol].[colno] = [pubdep].[colno] LEFT JOIN [regstusem] ON [regstusem].[stuno] = [stufile].[stuno] LEFT JOIN [pubcod] ON [pubcod].[codekind] = '08' AND [pubcod].[code] = [regstusem].[clacod] LEFT JOIN [selstchf] ON [regstusem].[syear] = [selstchf].[syear] AND [regstusem].[sem] = [selstchf].[sem] AND [stufile].[stuno] = [selstchf].[stuno] LEFT JOIN [con_behavior] ON [regstusem].[syear] = [con_behavior].[syear] AND [regstusem].[sem] = [con_behavior].[sem] AND [stufile].[stuno] = [con_behavior].[stuno] WHERE [stufile].[stuno] = '{StudentId}' AND [regstusem].[syear] = '{syearEnd}' AND [regstusem].[sem] = '{semEnd}'";
                ...
                using (SqlCommand command = new SqlCommand(cmd, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            stu.Add(new informStudent()
                            {
                                Acad = acad,
                                AcadName = reader.GetString(0),
                                Stuno = StudentId,
                                College = reader.GetString(1),
                                Department = reader.GetString(2),
                                Grade = cNumber[Int32.Parse(reader.GetString(3)) - 1].ToString() + "年",
                                Clacod = reader.GetString(4),
                                Name = reader.GetString(5),
                                Issued = DateTime.Now.ToString("yyy 年 M 月 d 日", culture),
                                DataYear = syearEnd,
                                DataSemester = semEnd,
